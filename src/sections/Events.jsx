          ...
          <Reveal key={e.title.en + e.year} delay={(i % 6) * 50}>
            <a href={e.url} target={e.url && e.url !== '#' ? '_blank' : undefined} rel="noreferrer"
              ...
              <div className="min-w-0 flex-1">
                <h3 className="text-[15px] font-medium text-neutral-900 dark:text-neutral-100 leading-snug group-hover:text-(--accent-ink) transition-colors">
                  {e.title[lang]}
                  <span className="ml-2 inline-flex items-center h-5 px-2 rounded-sm text-[10px] font-mono uppercase tracking-widest text-(--accent-ink) bg-(--accent)/10 border border-(--accent)/20 align-middle">
                    {e.role[lang]}
                  </span>
                </h3>
                <div className="mt-1 text-[13px] text-neutral-500 dark:text-neutral-400">{e.place}</div>
              </div>
              <span className="shrink-0 text-neutral-500 group-hover:text-(--accent-ink) group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all pt-1">
