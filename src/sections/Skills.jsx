          ...
          <Reveal key={g.group.en} delay={i * 60} className="h-full">
            <div className="group relative rounded-2xl border border-black/10 dark:border-white/10 p-6 bg-white/40 dark:bg-white/2 hover:border-(--accent)/40 transition-colors h-full flex flex-col">
              <div className="flex items-baseline justify-between mb-5">
                ...
                <span aria-hidden="true" className="text-[11px] font-mono text-neutral-500 dark:text-neutral-400">0{i + 1}</span>
              </div>
              <div className="flex flex-wrap gap-2 flex-1">
                {g.items.map((s, idx) => {
                  ...
                  const key = `${label}-${idx}`
                  return (
                    <span key={key} className="inline-flex items-center h-7 px-3 rounded-full text-[12px] text-neutral-700 dark:text-neutral-300 bg-neutral-100 dark:bg-white/5 border border-transparent hover:border-(--accent)/40 hover:text-(--accent-ink) transition-colors">{label}</span>
                  )
                })}
              </div>
            </div>
