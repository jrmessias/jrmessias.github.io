        ...
        {CV_DATA.projects.map((p, i) => (
          <Reveal key={p.url} delay={i * 80}>
            <a href={p.url} target="_blank" rel="noreferrer"
              className="group relative block overflow-hidden rounded-2xl border border-black/10 dark:border-white/10 bg-white/40 dark:bg-white/2 hover:border-(--accent)/50 transition-all hover:-translate-y-0.5">
              <div className="relative aspect-16/10 overflow-hidden bg-neutral-100 dark:bg-neutral-900">
                ...
                <div className="absolute inset-6 rounded-lg border border-black/10 dark:border-white/10 bg-white/60 dark:bg-neutral-950/60 backdrop-blur-xs flex flex-col">
                  <div className="flex items-center gap-1.5 px-3 h-6 border-b border-black/5 dark:border-white/5">
                    <span className="w-2 h-2 rounded-full bg-neutral-300 dark:bg-neutral-700" />
                    <span className="w-2 h-2 rounded-full bg-neutral-300 dark:bg-neutral-700" />
                    <span className="w-2 h-2 rounded-full bg-neutral-300 dark:bg-neutral-700" />
                    <span className="ml-2 text-[9px] font-mono text-neutral-400 truncate">{p.url.replace(/^https?:\/\//, '').replace(/\/$/, '')}</span>
                  </div>
                  <div className="flex-1 grid place-items-center px-6">
                    <div className="text-center">
                      <div className="text-2xl font-medium tracking-tight text-neutral-700 dark:text-neutral-300">{p.name}</div>
                      <div className="mt-2 text-[10px] font-mono uppercase tracking-[0.14em] text-(--accent)">{p.year}</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="p-6">
                ...
                <p className="mt-2 text-[14px] text-neutral-600 dark:text-neutral-400 leading-relaxed">{p.description[lang]}</p>
